...
import { ScreeningResult, Scan } from "@/src/types";

interface ScreeningItem extends Partial<Scan> {
  url: string;
  fileName: string;
  status: Scan["status"];
  isUploading?: boolean;
...
export default function ScreeningInterface({ dict }: { dict: Dictionary }) {
   ...
   */
  const runScreeningForItem = useCallback(
    async (index: number, itemId: string) => {
      // Simulate backend processing
      setTimeout(async () => {
        try {
          const docRef = doc(db, "scans", itemId);
          const mockResult = {
            prediction: "Negative",
            probabilities: [0.95, 0.05],
            heatmapUrl: null, // Heatmap would usually be generated by backend
          };
          ...
          setItems((prev) => {
            ...
            if (newItems[index]) {
              newItems[index] = {
                ...newItems[index],
                status: "done",
                result: mockResult as ScreeningResult,
              };
            }
            return newItems;
          });
  ...
  // Session state is now managed per-session in this component.
  // Permanent history is available on the History page.
  ...
  const handleUpload = async (uploadedFiles: File[]) => {
    ...
    // Upload each to Firebase Storage
      ...
      try {
        ...
        setItems((prev) => {
          ...
          if (next[targetIndex]) {
            next[targetIndex] = {
              ...next[targetIndex],
              id: scanDoc.id, // Using Firestore ID as the id
              url: downloadUrl,
              status: "analyzing",
            };
        ...
        runScreeningForItem(targetIndex, scanDoc.id);
      } catch (err: unknown) {
        const error = err as Error;
  ...
  const runBatchScreening = async () => {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      if (items[i].status !== "done" && itemId) {
        await runScreeningForItem(i, itemId);
      }
