...
export default function AnalysisResults({
  ...
  const predictionIndex =
    typeof prediction === "number"
      ? prediction
      : prediction === "Negative" || prediction === "No DR"
        ? 0
        : prediction === "Referable DR"
          ? 2 // Default to Moderate for Referable
          : parseInt(prediction as string) || 0;
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="text-2xl font-extrabold tracking-tight">
              {classes[predictionIndex]}
            </div>

            <div className="text-xs text-zinc-500 font-medium uppercase tracking-wider">
              AI-Aided Preliminary Classification
            </div>
          </div>
        </div>
      </div>
